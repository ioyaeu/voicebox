...
- **Preset voices** — 50+ curated voices via Kokoro, Qwen CustomVoice, and
  Voxtral on Apple Silicon
  for when you don't want to clone (see [Preset Voices](/overview/preset-voices))
- **Voice changer** — RVC voice-to-voice conversion: re-voice an existing
  recording as another voice using a community `.pth` model (see
  [Voice Changer](/overview/voice-changer))
...
| **Kokoro** | Preset (50 voices) | 8 | 82M parameters, CPU realtime, lowest VRAM of any engine |
| **Voxtral 4B TTS** | Preset (20 voices) | 9 | Apple Silicon MLX preset voices |
