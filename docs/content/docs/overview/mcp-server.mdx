...
voicebox.speak({
  ...
  language?: "en",
  plain_text?: true,             // strip markdown (code fences, tables, links, emphasis) before TTS; default from binding
  max_chars?: 600,               // cap the spoken text on a sentence boundary (min 50); default from binding
  keep_audio?: true,             // keep the audio in History; temporary by default
})
```
...
- **Speech-friendly text** — `plain_text: true` drops fenced code and tables, keeps link labels,
  and unwraps headings, bullets and emphasis; `max_chars` keeps the longest leading run that
  fits, cut where the TTS chunker would cut (sentence end, then clause, then whitespace).
  Both run *before* any personality rewrite. Meant for agents that hand over a raw markdown
  answer — a Claude Code `Stop` hook passing `${last_assistant_message}`, for instance. A
  text that strips down to nothing (code only) is rejected rather than spoken.
- **Temporary agent speech** — audio is removed after playback by default, so routine agent
  notifications do not accumulate in History. Set `keep_audio: true` when the user asks to
  keep, save, export, or reuse the audio — for example when the prompt says "keep audio".
...
| `default_plain_text` | When true, strip markdown before TTS by default (see `plain_text`). |
| `default_max_chars` | Cap the spoken text by default (see `max_chars`); empty means no cap. |
...
`personality`, `language`, `plain_text`, `max_chars`, `keep_audio`. Returns a
`GenerationResponse` — the same shape as `POST /generate`.
