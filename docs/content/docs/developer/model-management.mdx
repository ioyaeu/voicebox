...
**TTS Models** — Eight engines covering zero-shot cloning and preset voices. Each engine may have one or more size variants.
...
| **Voxtral 4B TTS** | `voxtral` | `mlx-community/Voxtral-4B-TTS-2603-mlx-4bit` | 2.5 GB | MLX only | 9 |

On Apple Silicon, Qwen TTS uses MLX-optimized repos from `mlx-community` instead of the PyTorch repos. Chatterbox Multilingual, TADA, and Voxtral also use MLX-specific paths when the backend is `mlx`. The backend picks automatically via `get_backend_type()`.

## RVC System Models

RVC voice profiles use user-supplied community `.pth` checkpoints and optional `.index` files. Voicebox also manages the shared system models needed for feature extraction and pitch tracking:

| Model | Engine | Source | Size | Purpose |
|-------|--------|--------|------|---------|
| **ContentVec** | `rvc` | `lengyue233/content-vec-best` | 360 MB | Voice conversion features |
| **RMVPE** | `rvc` | `lj1995/VoiceConversionWebUI` | 180 MB | Pitch extraction |
| **Crepe full** | `rvc` | commit-pinned `torchcrepe` asset | 85 MB | Optional pitch extraction |
