...
- **`profiles`** — Voice profiles with `voice_type` discriminator (`cloned` | `preset` | `designed` | `rvc`), `preset_engine`, `preset_voice_id`, `default_engine`, and RVC chain settings.
