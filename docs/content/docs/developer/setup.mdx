...
<Callout type="warn">
  On macOS/Linux, some Python dependencies (e.g. `pyworld`) build from source and need a C++ toolchain — install Xcode Command Line Tools with `xcode-select --install` on macOS.
</Callout>
...
- **TTS engines** — auto-download on first generation. Sizes range from Kokoro (~350 MB) to larger Qwen, TADA, and Voxtral downloads
